...
def test_explore_citation_graph(mock_get, mock_ctx):
    ...
    result = explore_citation_graph("10.1000/182", ctx=mock_ctx)
...
@patch("academic_hunter.interfaces.mcp.tools.discovery.requests.get")
def test_explore_citation_graph_api_error(mock_get, mock_ctx):
    mock_get.side_effect = RuntimeError("API timeout")

    with pytest.raises(MCPToolError, match="API timeout"):
        explore_citation_graph("10.1000/182", ctx=mock_ctx)
    mock_ctx.error.assert_called()


@patch("academic_hunter.interfaces.mcp.tools.discovery.requests.get")
def test_explore_citation_graph_no_data(mock_get, mock_ctx):
    mock_response = MagicMock()
    mock_response.status_code = 200
...
def test_fetch_paper_by_doi(mock_discovery_hunter, mock_ctx):
    result = fetch_paper_by_doi("10.1234/test", mock_ctx)
    assert "Abstract of a groundbreaking paper" in result
    mock_ctx.info.assert_called()


def test_fetch_paper_by_doi_not_found(mock_discovery_hunter, mock_ctx):
    mock_discovery_hunter.fetch_abstract_by_doi.return_value = None

    result = fetch_paper_by_doi("10.1234/unknown", mock_ctx)
    assert "No abstract could be retrieved" in result


def test_fetch_paper_by_doi_error(mock_discovery_hunter, mock_ctx):
    ...
    with pytest.raises(MCPToolError, match="API unavailable"):
        fetch_paper_by_doi("10.1234/test", mock_ctx)
    mock_ctx.error.assert_called()
...
def test_fetch_multiple_abstracts(mock_discovery_hunter, mock_ctx):
    mock_discovery_hunter.fetch_abstract_by_doi.side_effect = (
        lambda doi: f"Abstract for {doi}"
    )

    dois = ["10.1/A", "10.2/B"]
    result = fetch_multiple_abstracts(dois, mock_ctx)
...
@patch("academic_hunter.interfaces.mcp.tools.discovery.requests.get")
def test_quick_topic_discovery(mock_get, mock_ctx):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "data": [
    ...
    result = quick_topic_discovery("government blockchain", mock_ctx)
...
@patch("academic_hunter.interfaces.mcp.tools.discovery.requests.get")
def test_quick_topic_discovery_no_results(mock_get, mock_ctx):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"data": []}
    mock_get.return_value = mock_response

    result = quick_topic_discovery("unknown", mock_ctx)
    assert "No results found" in result


@patch("academic_hunter.interfaces.mcp.tools.discovery.requests.get")
def test_quick_topic_discovery_api_error(mock_get, mock_ctx):
    mock_get.side_effect = RuntimeError("API limit exceeded")

    with pytest.raises(MCPToolError, match="API limit exceeded"):
        quick_topic_discovery("test", mock_ctx)
    mock_ctx.error.assert_called()
