...
Academic Hunter is an automated Systematic Literature Review (SLR) tool that combines keyword-based and semantic relevance scoring in a configurable hybrid pipeline. It queries 7 academic databases in parallel (ArXiv, Crossref, OpenAlex, Semantic Scholar, CORE, DBLP, DOAJ) and exposes 8 further sources on demand through MCP tools (Europe PMC, OpenCitations, Unpaywall, Lens.org, OpenAIRE, bioRxiv, medRxiv, DataCite), deduplicates results across the parallel sources, scores papers using both exact keyword matching and weighted sentence embeddings, clusters results by topic, detects novel/outlier papers, generates extractive summaries, projects embeddings onto a 2D research landscape via UMAP, and exports results in CSV, BibTeX, RIS, Markdown, PRISMA, and JSON formats. The tool exposes a Model Context Protocol (MCP) server with 35+ tools, 4 resources, and 2 prompt templates, enabling Large Language Models and agentic workflows to interact with the review pipeline autonomously. The server supports both stdio and SSE transport for local and remote agent access.

# Statement of Need

Systematic Literature Reviews are a cornerstone of evidence-based research, but the screening phase — identifying relevant papers from thousands of candidates — remains labor-intensive. While tools such as ASReview [@asreview2020] use active learning to prioritize screening, they require manually labeled data to train classifiers. Commercial platforms like Rayyan and Covidence offer team collaboration but are closed-source, require web access, and lack programmable APIs. A new generation of SLR tools emerged in 2025-2026 (JARVIS Research OS [@jarvis2026], prismAId [@prismaid2025], LatteReview [@lattereview2025], ReviewAid [@reviewaid2026]) that leverage large language models for screening and extraction, but all require GPU-backed API calls to GPT-4, Gemini, or Claude — limiting offline use and raising privacy concerns for sensitive research data. Several MCP servers for literature discovery (lit-review-mcp [@litreviewmcp], ydzat-literature-review-mcp, review-mcp) have also appeared, though none offer a complete SLR pipeline with configurable semantic scoring.

Academic Hunter fills this gap as the only open-source SLR tool that combines multi-source search (7 sources queried in parallel, 15 accessible in total), configurable semantic relevance scoring via Weight-Bleeding (a novel embedding-space technique that requires no GPU, no labels, and no LLM), topic clustering, novelty detection, extractive summarization, research landscape visualization, and — critically — a Model Context Protocol (MCP) server that enables an LLM agent to **autonomously discover the domain vocabulary, build the search configuration with semantic weights, and execute the full pipeline** without manual intervention. All running locally on CPU:

| Feature                                   | Academic Hunter    | ASReview | Rayyan | Covidence |
| ----------------------------------------- | ------------------ | -------- | ------ | --------- |
| Open source (MIT)                         | ✅                 | ✅       | ❌     | ❌        |
| Zero-shot (no labeled data)               | ✅                 | ❌       | ✅     | ✅        |
| Multi-source API search (7 parallel)      | ✅                 | ❌       | ❌     | ❌        |
| Semantic relevance scoring                | ✅ Weight-Bleeding | ❌       | ❌     | ❌        |
| MCP server (35+ tools, 4 resources)       | ✅                 | ❌       | ❌     | ❌        |
| Agentic SLR Configuration¹                | ✅                 | ❌       | ❌     | ❌        |
| Agentic RAG ready                         | ✅                 | ❌       | ❌     | ❌        |
...
¹ Agentic SLR Configuration: the MCP tools `quick_topic_discovery`, `update_config`, and `run_search` enable an LLM agent to autonomously discover domain jargon from Semantic Scholar, build a weighted config.json with anchors and technical weights, and execute the full search pipeline without manual config editing. No competing tool provides this discovery-to-execution cycle.
...
│  │7 sources │  │Keyword   │  │CSV,BibTeX│  │ ChromaDB   │    │
...
**LLM-based SLR tools (2025-2026).** A new generation of SLR tools leverages large language models for screening and extraction. JARVIS Research OS [@jarvis2026] (2026, MIT) is the most comprehensive: it combines multi-source search (5 databases), citation network analysis with PageRank, contradiction detection, ChromaDB semantic search, active learning, and an MCP Hub (15 tools). However, JARVIS requires Gemini API access for its core pipeline — it cannot operate offline. prismAId [@prismaid2025] (JOSS, 2025) focuses on AI-powered information extraction from PDFs but depends on GPT-4/Claude/Gemini API calls. LatteReview [@lattereview2025] (arXiv, 2025) and ReviewAid [@reviewaid2026] (JORS, 2026) offer LLM-driven screening and extraction but lack multi-source search, topic clustering, and novelty detection. Several MCP servers for academic literature (lit-review-mcp, ydzat-literature-review-mcp, review-mcp) focus on sub-tasks such as citation snowballing, PDF summarization, or TF-IDF filtering — none offers the complete SLR pipeline with configurable semantic scoring. Academic Hunter is unique among 2025-2026 tools in providing: (a) a complete SLR pipeline that runs fully offline with no GPU and no LLM API calls; (b) Weight-Bleeding, a configurable semantic scoring method that requires no fine-tuning or labeled data; (c) 7 academic databases queried in parallel (15 accessible in total through MCP tools) — the widest multi-source coverage; and (d) the largest MCP tool surface (35+ tools) for agentic research workflows.
...
- **Multi-source aggregation**: concurrent queries across 7 academic APIs and preprint servers with domain-level rate limiting and exponential backoff.
...
- **Agentic SLR Configuration**: MCP tools `quick_topic_discovery`, `update_config`, and `run_search` form an autonomous discovery-to-execution cycle — the agent discovers domain jargon, builds a weighted configuration, and runs the pipeline without manual config editing.
...
Start the MCP server and drive the pipeline from any MCP-compatible agent:

```bash
academic-mcp               # start MCP server (stdio) for LLM agent integration
academic-mcp -t sse        # HTTP/SSE transport for remote access
```
...
We presented Academic Hunter, an open-source SLR tool that bridges traditional systematic review methodology with modern Agentic RAG infrastructure. Its Weight-Bleeding engine provides configurable embedding-space relevance scoring — a novel approach that biases bi-encoder embeddings toward domain-specific terms without modifying the model, requiring GPU compute, or needing labeled data. The tool's MCP server exposes 35+ retrieval and analysis tools, 4 resources, and 2 prompts that external LLM agents can orchestrate autonomously, including a unique **Agentic SLR Configuration** cycle where the agent discovers domain vocabulary, builds a weighted search configuration, and executes the full pipeline without manual intervention. This positions Academic Hunter as both a standalone SLR pipeline and an autonomous retrieval backbone for agentic research workflows. The same MiniLM embedding model that powers Weight-Bleeding also drives topic clustering, novelty detection, semantic deduplication, extractive summarization, and research landscape visualization — maximizing the value of a single 22MB model across 12 distinct analytical capabilities.
